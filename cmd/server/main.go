// main.go workbuddy-free 入口：加载配置、构建 pool、起调度器与 HTTP 服务。
//
// Patrick优化
package main
...
// 1.0.0 起为 workbuddy-free 新身份起点（此前的 1.8.1-panel 属 fork 血缘，不再沿用）。
...
func main() {
	...
	log.Printf("workbuddy-free listening on %s (api_key=%v)，管理面板 http://127.0.0.1%s/panel/", cfg.Listen, cfg.APIKey != "", panelListenPath(cfg.Listen))
