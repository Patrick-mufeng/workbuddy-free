...
// modelsPaths 按 realm 返回 CLI 模型目录端点路径候选序列：
// global → [v2, console]（国际站 /console 家族由 openresty 直接 500，只有 /v2 返回完整目录）；
// cn → [console]（现状逐字，零回归）。
// 判据与 globalModelsProbePaths（只取模型名）同家族，此处取完整能力表。
func (c *Client) modelsPaths(a *auth.Auth) []string {
	if c.globalOn(a) {
		return []string{"/v2/enterprises/personal/models", "/console/enterprises/personal/models"}
	}
	return []string{"/console/enterprises/personal/models"}
}

// fetchModelsOnce 单端点取模型目录原始 JSON。传输失败 / 非 2xx → err。
func (c *Client) fetchModelsOnce(a *auth.Auth, path string) ([]byte, error) {
...
// CLI 目录决定「能调哪些模型」（路径按 realm 分族，见 modelsPaths）；
// IDE /v3/config 覆盖同名模型的窗口 / 思考档（失败则静默保留 CLI 字段）。
func (c *Client) FetchModels(a *auth.Auth) ([]ModelInfo, error) {
	// 按 realm 试候选路径：任一 2xx 即用其响应，全失败返回最后一个错误。
	var (
		raw []byte
		err error
	)
	for _, path := range c.modelsPaths(a) {
		if raw, err = c.fetchModelsOnce(a, path); err == nil {
			break
		}
	}
	if err != nil {
